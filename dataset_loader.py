...
class FastSpeech2_dataset(Dataset):
    def __init__(self, text_path, model_config, preprocess_config):
        ...
        self.mel_channels = preprocess_config["preprocessing"]["mel"]["n_mel_channels"]
    ...
    def get_all(self, data):
        basename, speaker, phone, raw_text, emotion, act = data.split("|")
        if speaker == "LJSpeech":
            speaker = 1
    ...
    def collate_fn(self, batch):
        ...
        mel_length = [batch[i][5] for i in range(batch_size)]
        max_mel_length = max(mel_length)
        speaker = torch.tensor([int(batch[i][3]) for i in range(batch_size)], dtype = torch.int)


        text_pad = torch.zeros((batch_size, max_text_length), dtype = torch.long)
...
class VoiceAgent_dataset(Dataset):
    def __init__(self, text_path, model_config, preprocess_config):
        ...
        self.text_emb_dim = model_config["VoiceAgent"]["text_dim"]
    ...
    def get_mel(self, basename):
        ...
        mel_length = np.shape(mel)[0]
        return mel, pitch, energy, duration, mel_length, sbert
    ...
    def load_historical(self, basename):
        ...
        h_speaker, sbert_list = self.history_padding(h_speaker, sbert_list)
        return sbert_list, h_speaker, len(history_basename)

    def history_padding(self, speaker, sbert):
        h_length = len(speaker)
        _, channels = sbert.size()
        c_length = self.history_length - h_length
        speaker = [self.n_speakers for _ in c_length] + speaker
        sbert = [torch.zeros((c_length, self.text_emb_dim), dtype = torch.float)] + sbert
        return speaker, sbert

    def get_all(self, data):
        basename, speaker, phone, raw_text, emotion, act = data.split("|")
        phone, text_length = self.get_text(phone)
        mel, pitch, energy, duration, mel_length, sbert = self.get_mel(basename)
        h_sbert, h_speaker, h_length = self.load_historical(basename)
        return basename, phone, text_length, speaker, mel, mel_length, pitch, energy, duration, sbert, h_sbert, h_speaker, h_length
    ...
    def collate_fn(self, batch):
        """
        :param batch: [basename, phone, text_length, speaker, mel,
        mel_length, pitch, energy, duration, sbert,
         h_sbert, h_speaker, h_length]
        :return:
        """
        batch_size = len(batch)
        basename = [batch[i][0] for i in range(batch_size)]
        ...
        h_speaker = torch.tensor([batch[i][11] for i in range(batch_size)], dtype = torch.int)
        h_length = [batch[i][12] for i in range(batch_size)]

        text_pad = torch.zeros((batch_size, max_text_length), dtype=torch.float)
        c_emb = torch.zeros((batch_size,self.text_emb_dim), dtype = torch.float)
        h_emb = torch.zeros((batch_size, self.history_length, self.text_emb_dim), dtype = torch.float)
        mel_pad = torch.zeros((batch_size, max_mel_length, self.mel_channels), dtype=torch.float)
        pitch_pad = torch.zeros((batch_size, max_text_length), dtype=torch.float)
        energy_pad = torch.zeros((batch_size, max_text_length), dtype=torch.float)
        duration_pad = torch.zeros((batch_size, max_text_length), dtype=torch.int)
        ...
        for i in range(batch_size):
            ...
            energy_pad[i, : text_length[i]] = torch.tensor(batch[i][7], dtype=torch.float)
            duration_pad[i, :text_length[i]] = torch.tensor(batch[i][8], dtype=torch.float)
            c_emb[i] = torch.tensor(batch[i][9], dtype = torch.float)
            h_emb[i] = torch.tensor(batch[i][10], dtype = torch.float)

        return (
        basename, text_pad, text_length, max_text_length,
        speaker, mel_pad, mel_length, max_mel_length, pitch_pad,
        energy_pad, duration_pad, c_emb, h_emb, h_speaker, h_length)
